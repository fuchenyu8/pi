...
# === 模拟参数 ===
tariff_changes = [0.1, 0.2, 0.3]  # 关税变化幅度
exchange_rate_changes = [-0.03, 0.0, 0.05]  # 汇率变化幅度
domestic_substitution_rate = 0.5  # 国内替代率（初始值，50%）
random_noise_std = 0.05  # 随机波动标准差
...
def supply_function(base_supply, tariff, exchange_rate, alpha, gamma, rho, noise=0):
    international_supply = base_supply * (1 + alpha * tariff + gamma * exchange_rate)  # 国际供给
    domestic_supply = rho * (base_supply - international_supply)  # 国内替代
...
for scenario in ["Optimistic", "Baseline", "Pessimistic"]:
    beta_growth = 0.03 if scenario == "Optimistic" else 0.05 if scenario == "Baseline" else 0.07
    alpha, gamma = (0.2, -0.1) if scenario == "Optimistic" else (0.3, -0.2) if scenario == "Pessimistic" else (0.25, -0.15)
    
    for year, base_size in zip(years, base_market_size):
        beta = beta_function(year, beta_growth=beta_growth)
        ...
        for tariff in tariff_changes:
            for exchange_rate in exchange_rate_changes:
                ...
                adjusted_supply = supply_function(base_size, tariff, exchange_rate, alpha, gamma, domestic_substitution_rate, noise=noise_supply)
...
plt.title("Market Size Prediction with Dynamic Factors", fontsize=14)
